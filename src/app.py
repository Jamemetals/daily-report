...
# Load dataset
df = pd.read_csv(
    r"https://raw.githubusercontent.com/Jamemetals/daily/main/csq_matlab.csv?token=GHSAT0AAAAAACAITNMTYLEWHGLM7OR5IFOAZA6THKQ")
...
def kW_each_power_meter_day(df):
    kW = px.area(df, x="Time", y=["DP-CH-1 MAIN (kW)", 'DP-CH-2 MAIN (kW)', 'DP-CH-3 MAIN (kW)', 'DP-CH-4 MAIN (kW)',
                                  'DP-CH-5 MAIN (kW)'])
    kW.update_layout(
...
def pie_plot_kW_TR_day(df):
    ...
    DPCH = go.Pie(labels=pie1.columns, values=pie1.loc[0].tolist(), textinfo='percent', insidetextorientation='radial',
                  marker=dict(colors=px.colors.qualitative.Plotly))
    BTU = go.Pie(labels=pie2.columns, values=pie2.loc[0].tolist(), textinfo='percent', insidetextorientation='radial',
                 marker=dict(colors=px.colors.qualitative.Plotly_r)
                 )
...
def create_box_plot_day(df):
    df['Type_of_Peak'] = df['Time'].apply(
        lambda x: 'On-Peak' if pd.to_datetime('20/3/2023 8:45') < x < pd.to_datetime('20/3/2023 22:00') else 'Off-Peak')
    fig = go.Figure()
...
def summary_day(df):
    data_matrix = [
        [' ', "DP-CH-1 MAIN (kW)", 'DP-CH-2 MAIN (kW)', 'DP-CH-3 MAIN (kW)', 'DP-CH-4 MAIN (kW)', 'DP-CH-5 MAIN (kW)'],
        ['kW_mean', df["DP-CH-1 MAIN (kW)"].mean(), df["DP-CH-2 MAIN (kW)"].mean(), df["DP-CH-3 MAIN (kW)"].mean(),
         df["DP-CH-4 MAIN (kW)"].mean(), df["DP-CH-5 MAIN (kW)"].mean()],
        ['Max_kW', df["DP-CH-1 MAIN (kW)"].max(), df["DP-CH-2 MAIN (kW)"].max(), df["DP-CH-3 MAIN (kW)"].max(),
         df["DP-CH-4 MAIN (kW)"].max(), df["DP-CH-5 MAIN (kW)"].max()],
        ['Total kW', df["DP-CH-1 MAIN (kW)"].sum(), df["DP-CH-2 MAIN (kW)"].sum(), df["DP-CH-3 MAIN (kW)"].sum(),
         df["DP-CH-4 MAIN (kW)"].sum(), df["DP-CH-5 MAIN (kW)"].sum()]
        ]
...
app.layout = html.Div(
    [
        dbc.Row([
            html.H1('Power and BTU meter Data Visualization Dashboard (20 Mar 2023)',
                    style={'color': 'blue', 'text-align': 'center'})
        ]),
        dbc.Row(
            [dbc.Col(
                [dcc.Graph(figure=kW_each_power_meter_day(df), style={'width': '90vh', 'height': '50vh'})]
            ), dbc.Col(
                [dcc.Graph(figure=compare_kW_TR_day(df), style={'width': '90vh', 'height': '50vh'})]
            )]
        ),
        dbc.Row(
            [dbc.Col(
                [dcc.Graph(figure=pie_plot_kW_TR_day(df), style={'width': '90vh', 'height': '50vh'})]
            ), dbc.Col(
                [dcc.Graph(figure=create_box_plot_day(df), style={'width': '90vh', 'height': '50vh'})]
            )]
        ),
        dbc.Row(
            [dbc.Col(
